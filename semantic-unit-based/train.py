...
datas = torch.load(config.data)  # 可以用torch导入的data路径
print('loading time cost: %.3f' % (time.time()-start_time))

trainset, validset = datas['train'], datas['test']
...
if config.schedule:
    scheduler = L.ExponentialLR(optim.optimizer, gamma=0.5)
...
# 运行完下句后scores[metric]为一个列表, 即对应的指标值
    ...
    for raw_src, src, src_len, raw_tgt, tgt, tgt_len in trainloader:
        ...
        if updates % config.eval_interval == 0:
            ...
            for metric in config.metric:
                ...
                if metric == 'macro_f1' and score[metric] >= max(scores[metric]):  # 保存该项指标效果最好的模型，有几项指标就有几个模型
