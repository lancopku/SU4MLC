...
# def multilabel_loss(hidden_outputs, decoder, targets, criterion, config, sim_score=0):
#     outputs = hidden_outputs.view(-1, hidden_outputs.size(2))
#     scores = decoder.compute_score(outputs)
#     scores_sum = scores.sum(0).squeeze(0)
#     one_hot = torch.zeros(scores.size()).scatter_(1, targets, 1)
#     targets_sum = 
#     loss = criterion(outputs_sum, targets_sum) + sim_score
#     pred = scores.max(1)[1]
#     num_correct = pred.data.eq(targets.data).masked_select(targets.ne(dict.PAD).data).sum()
#     num_total = targets.ne(dict.PAD).data.sum()
#     loss.div(num_total).backward()
